...
func MyInspectDatabase(db ethdb.Database, keyPrefix, keyStart []byte) [][]string {
	it := db.NewIterator(keyPrefix, keyStart)
	defer it.Release()
	...
	filepath := common.Path + "Inspectdb" + strconv.Itoa(common.GlobalBlockNumber) + "_elapsed_" + common.PrettyDuration(time.Since(start)).String() + ".txt"
	if f, err := os.Create(filepath); err != nil {
		...
		fmt.Println("Inspecting database", "elapsed", common.PrettyDuration(time.Since(start)).String(), "file: ", filepath)
	}
	if unaccounted.size > 0 {
		log.Error("Database contains unaccounted data", "size", unaccounted.size, "count", unaccounted.count)
	}
