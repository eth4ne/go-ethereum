...
num_block = 305 # number of blocks to make at once
...
def run(num):
  ...
  for i in range(num):
    ...
    for j in range(NUM_NORMAL_TX): # for each tx in one block
      # 3, 4
      if (i+1)%3!=0:
        to = intToAddr(int(OFFSET_ADDR+2+j))
        amount = int(1)#int(OFFSET_ADDR+2+j)
      # 1, 2
      else:
        to = intToAddr(int(OFFSET_ADDR+j))
        amount = int(1)#int(OFFSET_ADDR+j)
      tx = {
        'from': coinbase,
        'to': to,
        'value': amount,
...
def makeRestoreTx(web3, currentBlock, address, gasprice):
  ...
  # else:
  #   latestCheckPoint = currentBlock - epoch
  ...
  print(f"latestCheckPoint: {latestCheckPoint}")
