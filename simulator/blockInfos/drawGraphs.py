...
def drawNodeStatGraphs(xAxis, yAxis1, yAxis2, yAxis3, xLabelName, yLabelName, graphTitle):
    ...
    # select graph type
    # plt.plot(blockNums, totalStateTrieNodeSizes) # draw plot
    # plt.scatter(blockNums, totalStateTrieNodeSizes, s=1) # draw scatter graph
...
def drawGraphsForBlockInfos(startBlockNum, endBlockNum, deleteEpoch=0, inactivateEpoch=0, inactivateCriterion=0):
    ...
    logFileName = ""
    if isEthane:
        logFileName += "ethane_simulate_block_infos_" + str(startBlockNum) + "_" + str(endBlockNum) \
        + "_" + str(deleteEpoch) + "_" + str(inactivateEpoch) + "_" + str(inactivateCriterion) + ".txt"
    else:
        logFileName += "ethereum_simulate_block_infos_" + str(startBlockNum) + "_" + str(endBlockNum) + ".txt"
    ...
    # parse log file
    totalStateTrieNodeSizes = []
    f = open(logFilePath+logFileName, 'r')
    rdr = csv.reader(f)
    blockNum = 0
    for line in rdr:
        if len(line) == 0:
        ...
        params = [int(x) for x in params] # covert string to int
        # print("params:", params)
