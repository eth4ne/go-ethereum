...
func InspectDatabase_save(db ethdb.Database, keyPrefix, keyStart []byte, blockNum int64) error {
	it := db.NewIterator(keyPrefix, keyStart)
	defer it.Release()
	...
	f, _ := os.OpenFile("inspected.txt", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	fmt.Fprint(f, "\ninspecting database at block ")
	fmt.Fprintln(f, blockNum)
	fmt.Fprintln(f, time.Now())
	for i:=0;i<len(stats);i++{
		fmt.Fprintln(f, stats[i])
	}
	total_string := "Total " + total.String()
	fmt.Fprintln(f, total_string)
